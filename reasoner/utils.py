...
class InterestLink():
    ...
    def discharge_depth(self, facts):
        if Sequent(self.conclusion.hypotheses, self.conclusion.conclusion) in facts:
            """print("ON REGARDE CE MEC", self.conclusion.hypotheses, "T", self.conclusion.conclusion)
            print()
            print("FACTS ACTUELS", facts)
            print("---")
            print("Avec les profondeurs", facts.depths)"""
...
def get_proof_tree(proof_dict, target):
    ...
    while queue != []:
        ...
        current = queue.pop(0)
        #print(current)
        #print(current in proof_dict, current not in seen)
        #input()
