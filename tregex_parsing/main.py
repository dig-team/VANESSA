...
class Tregex:
    ...
    def matches(self, node, variables, originalTree):
        newVariables={}
        """print("-----")
        print(self.label, [str(cond) for cond in self.conditions])
        print(node.label())
        print(node.indexes, [c.indexes for c in node])
        print(originalTree)
        print("-----")"""
    ...
    def matchesAnyRSister(self, node, variables, originalTree):
        """print("originalTree", originalTree)
        print(node.indexes)
        print(originalTree.get_node(node.indexes[:1]))"""
