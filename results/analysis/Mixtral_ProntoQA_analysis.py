...
def check_steps(dataset):
    ...
    for i, instance in enumerate(dataset):
        steps_result = []
        if instance["predicted_steps"] == ["No steps found in reasoning."]:
            continue
        for j, p in enumerate(instance["predicted_steps"]):
            ...
            if p == True:
                ...
                if truth in {"y","i"}:
                    ...
                    dico = {"id": instance["id"], "reasoning":instance["text_reasoning"], "predicted_steps":instance["predicted_steps"], "ground_truth_steps":instance["ground_truth_steps"], "entailments":instance["entailments"][j], "log_premises":instance["log_premises"][j], "log_conclusion":instance["log_conclusion"][j], "correspondance":instance["correspondance"][j]}
                    with open("FN-3110.jsonl", "a") as f:
                        pass
                        f.write(json.dumps(dico, ensure_ascii=False)+"\n")
                else:
                    ground_truth_counts[False] += 1
...
def check_reasonings(dataset):
    ...
    for i, instance in enumerate(dataset):
        ...
        if instance["predicted_reasoning"]:
            perfects.append(i)
            #print(i, instance["text_reasoning"], instance["predicted_steps"], instance["ground_truth_steps"], instance["ground_truth_reasoning"])
    ...
    print("Perfects", perfects, len(perfects), len(perfects_reasonings), perfects_reasonings)
    return steps_based_reasonings, text_based_reasonings, len(perfects_reasonings)/len(perfects)
...
def get_f1(file):
    dataset = load_data(file, "Mixtral_ProntoQA_annotations.jsonl")
    ...
    try:
        ...
        print("Steps:", accuracy, precision, recall, 0, 0)   
...
def get_all_f1s():
    print("-----")
    file = "../reasoning/Mixtral_ProntoQA-Symbolic-05_25.jsonl"
    print("Symbolic")
    get_f1(file)

    print("-----")
    file = "../reasoning/Mixtral_ProntoQA-LLaMa3-05_23.jsonl"
    print("LLaMa3")
    get_f1(file)

    print("-----")
    file = "../reasoning/Mixtral_ProntoQA-Deberta-05_24.jsonl"
    print("Deberta")
    get_f1(file)

    print("-----")
    file = "../reasoning/Mixtral_ProntoQA-LLaMa3-direct-11_14.jsonl"
    print("LLaMa3-direct")
    get_f1(file)

    print("-----")
    file = "../reasoning/Mixtral_ProntoQA-Deberta-direct-11_14.jsonl"
    print("Deberta-direct")
    get_f1(file)

    print("-----")
    file = "../reasoning/Mixtral_ProntoQA-GPT-direct-12_14.jsonl"
    print("GPT direct")
    get_f1(file)
