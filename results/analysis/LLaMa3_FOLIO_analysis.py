        ...
        for j, p in enumerate(instance["predicted_steps"]):
            ...
            if p == True:
                ...
                if truth in {"y","i"}:
                    ...
                    TP.append(total_count)
                else:
                    ground_truth_counts[False] += 1
                    steps_result.append("FP")
                    FP.append(total_count)
                    dico = {"id": instance["id"], "step_id": j, "reasoning":instance["text_reasoning"], "predicted_steps":instance["predicted_steps"], "ground_truth_steps":instance["ground_truth_steps"], "entailments":instance["entailments"][j], "log_premises":instance["log_premises"], "log_conclusion":instance["log_conclusion"], "correspondance":instance["correspondance"][j]}
                    with open("EA/FP_2025.jsonl", "a") as f:
                        #pass
...
def check_reasonings(dataset):
    ...
    return steps_based_reasonings, text_based_reasonings, len(perfects_reasonings)/perfects_count if perfects_count != 0 else 0
...
def get_f1_scores(steps_results, reasonings):
    ...
    try:
        ...
        reasoning_precision, reasoning_recall, reasoning_accuracy = 0, reasonings["TP"]/(reasonings["TP"]+reasonings["FN"]), (reasonings["TP"]+reasonings["TN"])/(reasonings["TP"]+reasonings["FP"]+reasonings["FN"]+reasonings["TN"])
    ...
    try:
        ...
        #f1 = wilson_conf_interval((steps["TP"]/(steps["TP"]+steps["FN"]))/(steps["FP"]/(steps["FP"]+steps["TN"])), steps["TP"]+steps["FN"]+steps["FP"]+steps["TN"])
        somers_d = (steps["TP"]*steps["TN"]-steps["FP"]*steps["FN"])/((steps["TP"]+steps["FP"])*(steps["TN"]+steps["FN"])*(steps["TN"]+steps["FP"])*(steps["TP"]+steps["FN"]))**0.5
    except ZeroDivisionError:
        precision, recall, accuracy = 0, steps["TP"]/(steps["TP"]+steps["FN"]), (steps["TP"]+steps["TN"])/(steps["TP"]+steps["FP"]+steps["FN"]+steps["TN"]), 0
...
def get_f1(file):
    dataset = load_data(file, "LLaMa3_FOLIO_annotations_full.jsonl")
...
def get_all_f1s():
    print("-----")
    file = "../reasoning/LLaMa3_FOLIO-Symbolic-05_25.jsonl"
    print("Symbolic")
    get_f1(file)

    print("-----")
    file = "../reasoning/LLaMa3_FOLIO-LLaMa3-05_22.jsonl"
    print("LLaMa3")
    get_f1(file)

    print("-----")
    file = "../reasoning/LLaMa3_FOLIO-Deberta-05_22.jsonl"
    print("Deberta")
    get_f1(file)

    print("-----")
    file = "../reasoning/LLaMa3_FOLIO-LLaMa3-direct-10_11.jsonl"
    print("LLaMa3 direct")
    get_f1(file)

    print("-----")
    file = "../reasoning/LLaMa3_FOLIO-Deberta-direct-10_11.jsonl"
    print("Deberta direct")
    get_f1(file)

    print("-----")
    file = "../reasoning/LLaMa3_FOLIO-GPT-direct-12_14.jsonl"
    print("GPT direct")
    get_f1(file)
